...
// const model = genAI.getGenerati  veModel({ model: "gemini-2.0-flash" });
...
const generateImageUrl = async (prompt: string): Promise<string> => {
  ...
  `;
    ...
    return JSON.parse(response).data[0]?.url || "https://placehold.co/480/png";
  } catch (error) {
    console.error("Failed to generate image:", error);
    return "https://placehold.co/480/png";
  }
};
...
`;
  ...
  try {
    ...
    try {
      jsonResponse = JSON.parse(text.replace(/```json|```/g, ""));
...
export const generateLayouts = async (projectId: string, theme: string) => {
  try {
    ...
    return {
      status: 500,
      error: "Error generating layouts",
      data: [],
    };
  }
};
